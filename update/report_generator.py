...
def generate_report_html(diagnosis_id: int, bpm_id: str, result: dict, created_at: str) -> str:
    ...
    # 审计材料清单
    checklist_html = ""
    for mat in checklist:
        checklist_html += f'<div class="checklist-item">☐ <strong>{mat["item"]}</strong><span class="mat-purpose"> — {mat["purpose"]}</span></div>'
...
<div class="page">
  ...
  <div class="checklist-box">
    {"".join([f'<div class=\"checklist-item\">☐ <div><strong>{m["item"]}</strong><div class=\"mat-purpose\">{m["purpose"]}</div></div></div>' for m in checklist]) if checklist else '<p style="color:#94a3b8">无需特别准备，保持常规过程留痕即可。</p>'}
  </div>
