...
bullet("列收模式判定（集团 27 号文）：按集团白名单 + 金额/利润率门槛 + 控制权，级联判定项目/单元该全额还是净额列收")
...
h2("4.1 什么是核算单元")
body("同一笔合同（同一个 BPM 商机）内，可能包含设备采购、施工、服务、标品（成品软件）等多个被分别核算的业务块，每块称为一个「核算单元」。不同类型的核算单元在列收判定中扮演不同角色：")
bullet("设备 / 标品类：默认按净额；但如属集团白名单，且项目金额/利润率门槛与控制权齐备，可全额列收（详见 4.6 列收模式信息）")
bullet("施工类：默认净额（施工本质不进白名单全额）")
bullet("服务类：是合规审查的重点，会触发相关规则检测，并做「硬转服务」实质识别")
doc.add_paragraph()
body("核算单元切分的意义在于：① 让系统按集团 27 号文的「列收模式」逐项判定每块该全额还是净额；② 避免把设备的低毛利混入服务侧毛利导致误报；③ 精准识别「申报为服务、实质为硬件」的包装风险（硬转服务）。")
doc.add_paragraph()
add_tip("重要变化：过去「设备/施工铁律一律不列收」的旧口径已按集团 27 号文调整——白名单标准化设备/成品软件在门槛与控制权齐备时可全额列收。某个单元最终全额还是净额，由系统的列收模式分类器算出（不再是手工选择）。")

h2("4.2 AI 切分草稿 → 用户确认")
body("信息收集完成后，系统会在对话区提示「建议切分核算单元」。点击后 AI 自动生成切分草稿，展示在右侧面板的「核算单元」区域。")
body("请逐项核对各单元的：申报类型、金额、毛利、是否有电信自有能力。对「设备 / 标品」单元，卡片上还会出现「集团白名单」选择项（是 / 否 / 不确定）——请按是否属于 27 号文集团白名单标准化设备/成品软件如实标注；不确定时系统从严按非白名单处理，确认后或可改善。如有误差可直接修改，确认无误后点击「确认核算单元」。")
doc.add_paragraph()
add_tip("提示：核算单元确认后会随诊断一起落库，报告中的「列收模式判定」「硬转服务嫌疑」等板块都基于此数据生成。单元「是否列收」不再手工选择，由系统按列收模式算出。")
...
h2("4.6 列收模式信息（集团 27 号文）")
body("「信息解析」面板还有一段「列收模式信息」，是按集团 27 号文判定项目该「全额列收」还是「净额列收」的关键输入。多为售中/财务信息，对话里常缺，需要你手动确认（AI 能明确识别的会自动填）：")
add_img(f"{OUT}/14_panel_listing_fields.png", width=Inches(4.5), caption="图 · 信息解析面板的「列收模式信息」段")
doc.add_paragraph()
bullet("是否为重大整合（单一组合产出）：硬件与服务深度耦合、电信做了重大定制、交付一个功能完整的单一系统才算「是」；笼统的「提供了集成服务」不算。它决定项目走「服务整合」还是「单一履约」路径")
bullet("项目整体税前利润率：含硬件的整体利润率（区别于服务侧毛利），按新政口径已剔除云/小微/基础业务后的值。喂全额门槛（服务整合≥10% / 单一履约≥5%）")
bullet("前向付款节点：是否为「首付款 + 到货验收尾款」——这是全额准入的硬条件，其他付款方式（分期/账期）不符合")
bullet("硬件产权是否验收后转移客户、后向集采比例：软提示项，不满足不一票否决，但需举证/补正")
bullet("是否为电信自投资设备打包（资本投资模式）：勾选后系统只做标记，收投比评估走线下投资流程")
doc.add_paragraph()
add_tip("提示：客户类型、付款节点不满足闭集会「硬否决」全额资格（落净额，可举证翻案）；产权、集采是「软提示」（不一票否决）。控制权是所有全额模式的总闸门——控制权不成立，硬件一律落净额。")
...
h2("5.3 列收模式判定（集团 27 号文）")
body("如项目含设备/标品/施工核算单元，报告会显示「列收模式判定」板块，是 27 号文逻辑的核心产出。它呈现：")
bullet("判定的列收模式：服务整合·全额 / 单一履约·白名单全额 / 收支差净额 / 资本投资（疑似，走线下评估）")
bullet("关键占比：硬件+集成施工占比（服务整合 ≤60%）、软硬件占比（单一履约 ≤80%）")
bullet("资格闸逐项 ✅/❌：签约额、占比、整体利润率、控制权等是否达标")
bullet("各核算单元的列收结论：每块标「全额列收」或「净额」")
bullet("硬否决（红，落净额可举证翻案）/ 软提示（黄，需举证补正不一票否决）")
doc.add_paragraph()
add_img(f"{OUT}/15_report_listing_mode.png", width=Inches(5.8), caption="图 · 报告「列收模式判定」板块")
doc.add_paragraph()
body("过去的「硬件铁律不列收」已按新政调整为「默认净额、白名单达标可全额」。申报为服务实质却是硬件的包装风险，仍由「硬转服务嫌疑」（见 5.4）单独识别——相关误申报规则（R24/R25）让位给单元级硬转服务检测。")
doc.add_paragraph()
add_tip("提示：工具止于「控制权判定 → 模式分类 → 差口举证」三层，不替你下财税科目/税率/预算结论。占比按填报口径与时点法（同一履约时间的非周期性收入）估算，若项目含运维年费等周期性收入，请人工剔除后复核。")
...
h3("Q9：以前设备一律不列收，现在为什么有的设备能全额列收了？")
body("这是按集团 27 号文（办政企〔2026〕27 号）调整的。新政下，集团白名单内的标准化设备/成品软件，在项目满足金额/利润率门槛、且电信对项目有控制权时，可以全额列收；不在白名单、或门槛/控制权不达标的，仍按净额。系统会在报告「列收模式判定」板块逐块给出结论和依据。请在核算单元卡片如实标注「集团白名单」，并在「列收模式信息」段补齐整体利润率、付款节点等。")
doc.add_paragraph()

h3("Q10：列收模式判定说「净额」，但我认为该全额，怎么办？")
body("工具的原则是「标风险、举证定生死，不替审核人定罪」。判定为净额时，板块会列出「硬否决」原因（如控制权未自证、付款节点不符）和需举证/补正的「软提示」。你可以补齐控制权角色、修正字段后重新提交；如确有依据维持全额，按板块提示准备举证材料，由审核人员据实判断。")
...
run = info.add_run("规则库版本 v1.8（集团 27 号文列收模式）· 广州电信云中台内部使用 · 2026 年 6 月")
